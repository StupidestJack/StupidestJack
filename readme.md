# 這裡是 StupidestJack
## 一個隨機刷新的中二
aka Niugnep / 豆子

> [!CAUTION]
> 別對我的專案抱太大希望，我造的東西基本都有AI幫忙awa<br>
> 除了作業pwp

---

### 關於我
原 [@Mamegoodbean2k](https://github.com/Mamegoodbean2k) 帳號使用者。<br>
14歲<br>
喜歡寫程式、看影片、玩麥塊、~~男娘~~

### 程式語言能力（咩比
提示：以下皆為signed short類型整數
| 語言 | 熟練度 | 備註 |
| :--- | :--- | :--- |
| **Python** | 70% | 太久沒完功力生疏了 |
| **C#** | 85% | Linq舒服 |
| **Visual Basic** | 40% | 英文不好慎入 |
| **JavaScript** | 60% | 難以理解1+"1"="11"|
| **Assembly** | 3% | 國二生會這個信嗎 |
| **Brainf_ck** | 0% | 跟組語很像但我不想寫 |
| **C** | 30% | 太好玩了吧啊啊啊 |
| **HTML** | -1% | 這不是程式語言啊 |
| **Scratch** | 32767%+1% | 這是玩具，禁止減一 |


### 傳送門
* [個人網站（半死狀態](https://stupidestjack.github.io/myWebsite/)
* [X](https://x.com/stupidestjack)
* [Discord伺服器](https://discord.gg/VQ9uNweSn9)
* 至於Line，哪個正常人會喜歡用，你知道的，Line這坨ㄉㄅ，功能爛廣告多耗電快空間肥，最近這臭不要臉的逼還出了個Premium，迫於現實壓力你又不能不用，真的是建議送給反壟斷
 
### 文件
[裝備](/docs/specs.md)<br>
[人類調教說明書](https://www.youtube.com/watch?v=dQw4w9WgXcQ)

### 版權申明
此Readme使用WTFPL (Do What The F\*\*k You Want To Do Public License) 開源，<br>
除了我的個資、網站、帳號等，<br>
以及本來就受著作權保護的Rickroll不開源


# SO NVIDIA F__K YOU
